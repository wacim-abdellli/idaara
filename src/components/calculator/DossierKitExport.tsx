...
import { Printer, Stamp, Clock, FileCheck2, Download, CheckCircle2, Loader2, Eye, X } from 'lucide-react';
...
export const DossierKitExport: React.FC<DossierKitExportProps> = ({
  ...
  return (
    <>
      ...
      {/* ── MODAL FULLSCREEN A4 PREVIEW LIGHTBOX ── */}
      <AnimatePresence>
        {showModal && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="relative max-w-4xl w-full max-h-[92vh] flex flex-col bg-zinc-950 border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden my-auto"
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-3.5 bg-zinc-900/90 border-b border-zinc-800 shrink-0">
                <div className="flex items-center gap-2 text-white text-xs font-bold">
                  ...
                  <span>{locale === 'ar' ? 'معاينة الورقة الرسمية A4' : 'Aperçu Document Officiel A4'}</span>
                </div>

                <div className="flex items-center gap-2">
                  <button
                    onClick={handlePrint}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold transition-colors cursor-pointer"
                  >
                    <Printer className="w-3.5 h-3.5 text-zinc-400" />
                    <span className="hidden sm:inline">{locale === 'ar' ? 'طباعة' : 'Imprimer'}</span>
                  </button>

                  <button
                    onClick={handleDownloadPDF}
                    disabled={isGenerating}
                    className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-zinc-950 text-xs font-bold transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
                  >
                    {isGenerating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
                    <span>{locale === 'ar' ? 'حفظ PDF' : 'Télécharger'}</span>
                  </button>

                  <button
                    onClick={() => setShowModal(false)}
                    className="p-1.5 rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-zinc-400 hover:text-white transition-colors cursor-pointer ml-1"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              {/* Modal Scaled A4 Document Container */}
              <div className="p-4 sm:p-8 overflow-y-auto overflow-x-auto flex justify-center bg-zinc-900/40">
                <div
                  className="w-full max-w-[794px] min-h-[1050px] bg-white text-zinc-900 p-8 sm:p-10 space-y-4 shadow-2xl rounded-sm text-left"
                  dir={isRtl ? 'rtl' : 'ltr'}
      ...
      {/* ── HIGH-RES A4 OFFICIAL DOSSIER CONTAINER (FOR PDF & PRINT ENGINE ONLY) ── */}
