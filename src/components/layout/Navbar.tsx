...
export const Navbar: React.FC = () => {
  ...
  const navLinks = [
    { href: '/copilot', label: t('copilotNav'), icon: Mic, badge: 'Voice' },
    { href: '/fasserli', label: t('fasserliNav'), icon: FileSearch, badge: 'OCR' },
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-zinc-800/80 bg-zinc-950/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-700 flex items-center justify-center shadow-lg shadow-emerald-950 group-hover:scale-105 transition-transform duration-200">
              <span className="text-xl">🏛️</span>
            </div>
            <div className="flex flex-col">
              <div className="flex items-center space-x-1.5">
                <span className="font-extrabold text-lg text-white tracking-tight">Idaara</span>
          ...
          <div className="hidden sm:flex items-center space-x-3">
            ...
            <Link
              href="/copilot"
              className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 text-xs font-bold shadow-md shadow-emerald-500/20 transition-all hover:scale-105"
            >
          ...
          <div className="flex lg:hidden items-center space-x-2">
            ...
            <button
              ...
              className="p-2 rounded-lg bg-zinc-900 text-zinc-400 hover:text-zinc-100 border border-zinc-800"
            >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-b border-zinc-800 bg-zinc-950/95 px-4 pt-2 pb-6 space-y-1">
          {navLinks.map((link) => {
            const isActive = pathname === link.href || (link.href !== '/' && pathname.startsWith(link.href));
            const Icon = link.icon;
            return (
