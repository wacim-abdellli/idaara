...
export const StatusComparator: React.FC = () => {
  ...
  const statuses = [
    {
      ...
      badge:
        locale === 'ar'
          ? 'موصى به للمستقلين والمبرمجين'
          : locale === 'derja'
          ? 'Recommandé Freelances & Devs'
          : locale === 'en'
          ? 'Recommended for Freelancers & Tech'
          : 'Recommandé Freelances & Devs',
      taxRate: `${aeRates.servicesTaxRate * 100}% (Services) / ${aeRates.commerceTaxRate * 100}% (Commerce)`,
      cnss:
        locale === 'ar'
          ? `مبلغ رمزي جزافي (~${aeRates.quarterlyCnssContributionTND} د.ت / 3 أشهر)`
          : locale === 'derja'
          ? `Forfaitaire symbolique (~${aeRates.quarterlyCnssContributionTND} DT / trimestre)`
          : locale === 'en'
          ? `Symbolic flat fee (~${aeRates.quarterlyCnssContributionTND} DT / quarter)`
          : `Forfaitaire symbolique (~${aeRates.quarterlyCnssContributionTND} DT / trimestre)`,
      comptable:
        locale === 'ar'
          ? 'غير مطلوب (منصة رقمية)'
          : locale === 'derja'
          ? 'Non requis (Plateforme en ligne)'
          : locale === 'en'
          ? 'Not required (100% digital portal)'
          : 'Non requis (Plateforme en ligne)',
      capital: '0 DT',
      facturation:
        locale === 'ar'
          ? 'فواتير بمعرف وطني QR'
          : locale === 'derja'
          ? 'Factures avec Matricule National QR'
          : locale === 'en'
          ? 'QR Code Invoicing with National ID'
          : 'Factures avec Matricule National QR',
      maxChiffreAffaire:
        locale === 'ar'
          ? `حتى ${formatNumber(aeRates.annualRevenueCeilingTND)} د.ت / سنة`
          : locale === 'derja'
          ? `Jusqu'à ${formatNumber(aeRates.annualRevenueCeilingTND)} DT / an`
          : locale === 'en'
          ? `Up to ${formatNumber(aeRates.annualRevenueCeilingTND)} TND / year`
          : `Jusqu'à ${formatNumber(aeRates.annualRevenueCeilingTND)} DT / an`,
    ...
    },
    {
      ...
      name:
        locale === 'ar'
          ? 'براءة شخص طبيعي (Patente)'
          : locale === 'derja'
          ? 'Patente Personne Physique'
          : locale === 'en'
          ? 'Sole Proprietorship (Patente)'
          : 'Patente Personne Physique',
      badge:
        locale === 'ar'
          ? 'المهن الحرة والحرفيون'
          : locale === 'derja'
      ...
      taxRate:
        locale === 'ar'
          ? 'جدول تصاعدي IRPP (حتى 35%)'
          : locale === 'derja'
          ? "Barème progressif IRPP (Jusqu'à 35%)"
          : locale === 'en'
          ? 'Progressive IRPP scale (Up to 35%)'
          : "Barème progressif IRPP (Jusqu'à 35%)",
      cnss:
        locale === 'ar'
          ? 'نظام المستقلين حسب الشريحة'
          : locale === 'derja'
          ? 'Régime des Indépendants (Palier déclaré)'
          : locale === 'en'
          ? 'Self-employed scale by declared bracket'
          : 'Régime des Indépendants (Palier déclaré)',
      comptable:
        locale === 'ar'
          ? 'موصى به / موازنة سنوية'
          : locale === 'derja'
          ? 'Recommandé / Bilan annuel simplifié'
          : locale === 'en'
          ? 'Recommended / Simplified balance sheet'
          : 'Recommandé / Bilan annuel simplifié',
      capital: '0 DT',
      facturation:
        locale === 'ar'
          ? 'فواتير بمعرف جبائي قباضة'
          : locale === 'derja'
          ? 'Factures avec Matricule Fiscal Recette'
          : locale === 'en'
          ? 'Invoices with Recette Tax ID'
          : 'Factures avec Matricule Fiscal Recette',
      maxChiffreAffaire:
        locale === 'ar' ? 'غير محدود' : locale === 'derja' ? 'Illimité' : locale === 'en' ? 'Unlimited' : 'Illimité',
    ...
    },
    {
      ...
      name:
        locale === 'ar'
          ? 'شركة الشخص الواحد (SUARL)'
          : locale === 'derja'
          ? 'Société SUARL (Personne Morale)'
          : locale === 'en'
          ? 'Single-Member LLC (SUARL)'
          : 'Société SUARL (Personne Morale)',
      badge:
        locale === 'ar'
          ? 'الشركات الناشئة والتصدير'
          : locale === 'derja'
          ? "Startups & Sociétés d'Export"
          : locale === 'en'
          ? 'Startups & Export Companies'
          : "Startups & Sociétés d'Export",
      ...
      cnss:
        locale === 'ar'
          ...
          : locale === 'en'
          ? 'Non-salaried majority manager'
          : 'Gérant majoritaire non salarié',
      comptable:
        locale === 'ar'
          ? 'إجباري (خبير محاسب)'
          : locale === 'derja'
          ? 'Obligatoire (Comptable agréé)'
          : locale === 'en'
          ? 'Mandatory (Certified Accountant)'
          : 'Obligatoire (Comptable agréé)',
      capital: `${formatNumber(suarlRates.minimumBankCapitalTND)} DT (Bloqué en banque)`,
      facturation:
        locale === 'ar'
          ? 'شركة تجارية بالسجل الوطني RNE'
          : locale === 'derja'
          ? 'Société commerciale RNE'
          : locale === 'en'
          ? 'Commercial entity registered at RNE'
          : 'Société commerciale RNE',
      maxChiffreAffaire:
        locale === 'ar' ? 'غير محدود' : locale === 'derja' ? 'Illimité' : locale === 'en' ? 'Unlimited' : 'Illimité',
      color: 'border-zinc-800 bg-zinc-900/60',
  ...
  const taxRateLbl =
    locale === 'ar' ? 'نسبة الضريبة :' : locale === 'derja' ? 'Taux dhariba :' : locale === 'en' ? 'Tax Rate:' : "Taux d'imposition :";
  const cnssLbl =
    locale === 'ar' ? 'الضمان الاجتماعي CNSS :' : locale === 'derja' ? 'Cotisation CNSS :' : locale === 'en' ? 'Social Security (CNSS):' : 'Cotisation CNSS :';
  const comptableLbl =
    locale === 'ar' ? 'المحاسب القانوني :' : locale === 'derja' ? 'Comptable :' : locale === 'en' ? 'Accounting Requirements:' : 'Tenue Comptable :';
  const capLbl =
    locale === 'ar' ? 'الرأسمال المطلوب :' : locale === 'derja' ? 'Capital lezem :' : locale === 'en' ? 'Required Minimum Capital:' : 'Capital Minimum Exigé :';
  const maxLbl =
    locale === 'ar' ? 'سقف رقم المعاملات :' : locale === 'derja' ? 'Plafond CA :' : locale === 'en' ? 'Annual Turnover Ceiling:' : "Plafond Chiffre d'Affaires :";

  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">{heading}</h3>
        <p className="text-xs sm:text-sm text-zinc-400 max-w-2xl">{sub}</p>
      </div>
